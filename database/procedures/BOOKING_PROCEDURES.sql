...
PROC_BLOCK: BEGIN
    ...
    SELECT ST.STATUSCODE, B.SERVICEDATE INTO VSTATUSCODE, VSERVICEDATE 
    FROM CMS_BOOKING B
    JOIN CMS_STATUS ST ON ST.STATUSID = B.STATUSID
    WHERE B.BOOKID = PBOOKID;
    ...
    IF VSERVICEDATE > CURDATE() THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cannot mark a future booking as no-show';
    END IF;
...
-- List bookings with optional filters (including Canteen scope)
DROP PROCEDURE IF EXISTS CMSLISTBOOK$$
...
-- Atomically pre-books meals for multiple days / an entire weekly pass
