...
DROP PROCEDURE IF EXISTS CMSCANWALLETWD;
...
BEGIN
    ...
    IF VTYPECODE NOT IN ('CNT', 'VIS', 'CONTEMP', 'VISITOR', 'CONT') THEN
...
BEGIN
    ...
    IF PPAGE IS NOT NULL AND PPAGESIZE IS NOT NULL THEN
        ...
        JOIN CMS_WALLET W ON W.WALLETID = WT.WALLETID
        WHERE W.CUSTOMERID = PCUSTOMERID
        ...
        ORDER BY WT.CREATEDAT DESC, WT.WALLETTRANID DESC
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE WD.WALLETID = VWALLETID
          AND ST.STATUSCODE = 'REQ'
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'A pending withdrawal request already exists for this wallet';
    END IF;
...
/* ============================================================
   PROCEDURE: CMSCANWALLETWD
   ------------------------------------------------------------
   Cancels a pending withdrawal by the owning customer
   and releases the reserved funds back to available balance.
   ============================================================ */
CREATE PROCEDURE CMSCANWALLETWD
(
    IN PWALLETWDID INT,
    IN PCUSTOMERID INT,
    IN PREMARKS VARCHAR(500)
...
BEGIN
    ...
    SELECT STATUSID INTO VCANSTATUSID 
    FROM CMS_STATUS 
    WHERE STATUSCODE = 'CAN' AND STATUSGRP = 'WITHDRAW_REQ' 
    LIMIT 1;

    IF VCANSTATUSID IS NULL THEN
        SELECT STATUSID INTO VCANSTATUSID FROM CMS_STATUS WHERE STATUSCODE = 'CAN' LIMIT 1;
    END IF;
    IF VCANSTATUSID IS NULL THEN
        SET VCANSTATUSID = 53;
    END IF;
    ...
    INTO
        ...
        VCUSTID
    FROM CMS_WALLETWD WD
    JOIN CMS_WALLET W ON W.WALLETID = WD.WALLETID
    JOIN CMS_STATUS ST ON ST.STATUSID = WD.STATUSID
    WHERE WD.WALLETWDID = PWALLETWDID
    FOR UPDATE;
    ...
    IF PCUSTOMERID IS NOT NULL AND VCUSTID <> PCUSTOMERID THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'You can only cancel your own withdrawal request';
    END IF;

    IF VSTATUSCODE <> 'REQ' THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Only requested withdrawals can be cancelled';
    END IF;
