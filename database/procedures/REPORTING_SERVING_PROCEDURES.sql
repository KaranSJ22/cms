...
BEGIN
    ...
    SELECT
        ...
        COALESCE(SUM(CASE WHEN ST.STATUSCODE = 'CR'           THEN B.TOTALAMOUNT ELSE 0 END), 0) AS AMOUNTPENDING,
        COALESCE(SUM(CASE WHEN ST.STATUSCODE IN ('SRV','NOS','CR') THEN B.TOTALAMOUNT ELSE 0 END), 0) AS TOTALCOMMITTED,
        COUNT(CASE WHEN ST.STATUSCODE IN ('SRV','NOS','CR') THEN 1 END)              AS TOTALBOOKINGS
    FROM CMS_BOOKING B
    JOIN CMS_STATUS ST ON ST.STATUSID = B.STATUSID
...
BEGIN
    ...
    SELECT STATUSID INTO VCRSTATUSID FROM CMS_STATUS WHERE STATUSCODE = 'CR' AND STATUSGRP = 'BKG';
