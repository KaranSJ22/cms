...
| Column     | Type          | Nullable | Default             | Constraints              |
|------------|---------------|----------|---------------------|--------------------------|
| SCREENID   | INT AI        | NO       | —                   | PRIMARY KEY              |
| SCREENCODE | VARCHAR(50)   | NO       | —                   | UNIQUE                   |
| SCREENNAME | VARCHAR(100)  | NO       | —                   |                          |
| DESCR      | VARCHAR(255)  | YES      | NULL                |                          |
| ROUTEPATH  | VARCHAR(150)  | YES      | NULL                |                          |
| ISACTIVE   | TINYINT(1)    | NO       | 1                   | CHECK (ISACTIVE IN (0,1))|
| CREATEDBY  | INT           | YES      | NULL                | FK → CMS_USER(USERID)   |
| CREATEDAT  | DATETIME      | NO       | CURRENT_TIMESTAMP   |                          |
| UPDATEDAT  | DATETIME      | YES      | ON UPDATE           |                          |

**Purpose**: Application screen configuration for RBAC. Used by `CMS_ROLESCN` to map role permissions to screens.
...
- `CMS_SCREEN.CREATEDBY` → `CMS_USER(USERID)` (Module 1 dependency)
